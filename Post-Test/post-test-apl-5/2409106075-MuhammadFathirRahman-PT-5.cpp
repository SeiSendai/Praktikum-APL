...
struct Akun {
    ...
    string serverTujuan;
    string password;
    User* user; // Mengubah menjadi pointer ke User
};
...
// Fungsi baru dengan parameter pointer
void updateUserRole(User* user, const string &newRole); // Menggunakan dereference
void incrementAkunCount(int* count); // Menggunakan address-of
...
// Fungsi yang menggunakan dereference parameter
void updateUserRole(User* user, const string &newRole) {
    if (user != nullptr) {
        user->role = newRole; // Dereference pointer untuk mengakses member
        cout << "Role user " << user->username << " diubah menjadi " << newRole << endl;
    }
}

// Fungsi yang menggunakan address-of parameter
void incrementAkunCount(int* count) {
    if (count != nullptr) {
        (*count)++; // Dereference pointer untuk increment nilai
...
int tambahAkun(Akun akun[], int &akunCount, User* user) {
    if (akunCount < MAX_AKUN) {
        ...
        akun[akunCount].user = user; // Menyimpan pointer ke user
